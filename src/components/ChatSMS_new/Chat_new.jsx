...
import { checkChat, getChatInfo, getChatMessages, getFriendsList, getUserInfo, sendMessageOne, sendMessageToChat } from "../../hooks/api";
import { InfoChat } from "./InfoChat/InfoChat";
import { AddUserChat } from "./Add_UserChat/Add_userChat";

let img_p = "/imgs/log/Group 25 (2).svg";

export const Chat_new = () => {
...
console.log(state.friend)
  ...
  useEffect(()=>{
    if(isChat.exists){
   getChatMessages(isChat.chatID, 20, 0)
  ...
  const handleSubmit_many = async () => {
    ...
    const handleSubmit_mew = async () => {
      const otherUsers = isInfo.filter((user) => user.user_id !== isUserInfo.id_user);
    try {
      await sendMessageOne({
        receiver_id: state.friend.id,
        message_text: isSms_,
        message_file: "",
      });
  ...
  return (
    <div className={styles.div_}>
      <div className={styles.heder_chat_div} onClick={() => {
        ...
        <button>naz</button>
        <img src={isUnfoChat?.pfoto||state.friend.pfoto || img_p} alt="Chat" />
        <div className={styles.heder_name_date}>
          <p style={{ fontSize: "15px" }}>{isUnfoChat?.name_chat??state.friend.login}</p>
        </div>
        <div></div>
      </div>
